...
/******** OWNER NUMBER**********/
const ownerNumber =  ["573175852207@s.whatsapp.net","573175852207@s.whatsapp.net"] 
/************************************/
...
const client = new WAConnection()
   ...
   console.log(color('[','white'),color('∆','red'),color(']','white'),color('Código listo','white'),color('SAMIR','red'),color('THANKS','white'),color('SAMIR','yellow'))
})
...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `@${num.split('@')[0]}\ Bienvenid@ *${mdata.subject}* Joder que bueno verte por aca😉Espero te la pases de lo mejor, te invito a ver las reglas del grupo para que no allá malos entendidos`
				...
				teks = `Hasta que te fuiste pedazo de aborto 😈 @${num.split('@')[0]}Ya no te queríamos ojala nunca regreses 👋bye`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
           ...
           return false
			...
			switch(command) { 
                ...
                case 'glitchtext':
					...
					var gli = gh.split("&")[0];
					var tch = gh.split("&")[1];
					if (args.length < 1) return reply(`「❗」Ejemplo : ${prefix}glitchtext Bot & 🥋𝒔𝒂𝒎𝒖𝒓𝒂𝒊 🤖 `)
					reply(ind.wait())
				...
				case 'tahta':
					if (!isRegistered) return reply(ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
					if (args.length < 1) return reply(`「❗」Ejemplo : ${prefix}hartatahta Sami`)
					har = body.slice(12)
					reply('「❗」Hartatahta')
            ...
            case 'bitly':
               ...
               if (data.error) return reply(data.error)
                ...
                var gh = body.slice(12)
					...
					buffer = await getBuffer(anu.result)
					client.sendMessage(from, buffer, image, {caption: 'Mr.🥋𝒔𝒂𝒎𝒖𝒓𝒂𝒊 🤖 ', quoted: mek})
					await limitAdd(sender)
					break
				//fadli 
				case 'pinterest':
               ...
               case 'help': 
         	   ...
         	   case 'owner':
         	   case 'creator':
                  client.sendMessage(from, {displayname: "Samir", vcard: vcard}, MessageType.contact, { quoted: mek})
...
• Limit: ${limitawal-updated.limit}`
            ...
            break
				...
				case 'slap':
					...
					client.sendMessage(from, pod, image, { quoted: mek, caption: '*Samir*\n\n'+ ple })
					await limitAdd(sender)
					break
					case 'tampar':
					if (!isRegistered) return reply(ind.noregis())
           ...
           if (isLimit(sender)) return reply(ind.limitend(pusname))
				...
				if (!isQuotedSticker) return reply('Responde a un sticker')
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						...
						client.sendMessage(from, buffer, image, {quoted: mek, caption: '🥋𝒔𝒂𝒎𝒖𝒓𝒂𝒊 🤖'})
						fs.unlinkSync(ran)
					})
					await limitAdd(sender)
				break 
